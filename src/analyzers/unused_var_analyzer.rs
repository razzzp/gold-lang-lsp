...
use crate::{
    ...
    parser::ast::{
        AstBinaryOp, AstFunction, AstLocalVariableDeclaration, AstProcedure, AstTerminal, IAstNode,
    },
...
impl IVisitor for UnusedVarAnalyzer {
    implem_as_ivisitor!();
    fn visit(&mut self, node: &DynamicChild<dyn IAstNode>) {
        ...
        match node.data.as_any().downcast_ref::<AstTerminal>() {
            Some(_data) => self.notify_terminal_node(node),
            _ => (),
        }
        match node
            .data
            .as_any()
            .downcast_ref::<AstLocalVariableDeclaration>()
        {
            Some(node) => self.notify_local_var_node(node),
            _ => (),
        }
    }
